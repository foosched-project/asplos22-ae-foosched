...
"""cli.py - A command line interface for foosched

Usage:
  foosched-cli init        <release_kernel> <kernel_src> <work_dir>
  foosched-cli dev_init    <kernel_src> <work_dir>
  foosched-cli extract_src <kernel_src_rpm> <target_dir>
  foosched-cli build       <work_dir>
  foosched-cli (-h | --help)
...
Available subcommands:
  ...
  dev_init      Initialize foosched development envrionment from kernel source code
...
class Foosched(object):
    def __init__(self, work_dir, vmlinux, makefile):
        self.foosched_path = os.path.dirname(os.path.realpath(__file__))
        ...
        foosched_sh = sh(_cwd=self.foosched_path)
        mod_sh = sh(_cwd=self.work_dir)
        self.foosched_sh, self.mod_sh = foosched_sh, mod_sh
        self.get_kernel_version(self.makefile)
        self.get_config_dir()
        self.search_springboard = sh.Command(self.foosched_path + '/tools/springboard_search.sh')
        ...
        self.file_mapping = {
            ...
            'src/Makefile.foosched':       self.tmp_dir,
    ...
    def get_kernel_version(self, makefile):
        VERSION = self.foosched_sh.awk('-F=', '/^VERSION/{print $2}', makefile).strip()
        PATCHLEVEL = self.foosched_sh.awk('-F=', '/^PATCHLEVEL/{print $2}', makefile).strip()
        SUBLEVEL = self.foosched_sh.awk('-F=', '/^SUBLEVEL/{print $2}', makefile).strip()
        self.KVER = '%s.%s.%s' % (VERSION, PATCHLEVEL, SUBLEVEL)

        KREL = self.foosched_sh.awk('-F=', '/^EXTRAVERSION/{print $2}', makefile).strip(' \n-')
        if len(KREL) == 0:
            logging.fatal('''Maybe you are using foosched on non-released kernel,
    ...
    def get_config_dir(self):
        ...
        candidates = list(map(os.path.basename, glob('%s/configs/%s*' % (self.foosched_path, self.major))))
        if len(candidates) == 0:
        ...
        self.config_dir = os.path.join(self.foosched_path, 'configs/', candidates[idx])
    ...
    def make(self, stage, objs=[], **kwargs):
        self.mod_sh.make(stage,
                         ...
                         *['%s=%s' % i for i in kwargs.items()],
                         file=os.path.join(self.tmp_dir, 'Makefile.foosched'),
                         jobs=self.threads,
                         _out=sys.stdout,
                         _err=sys.stderr)
    ...
    def extract(self):
        ...
        self.make(stage = 'collect', foosched_tmpdir = self.tmp_dir, foosched_modpath = self.mod_path)
        self.make(stage = 'analyze', foosched_tmpdir = self.tmp_dir, foosched_modpath = self.mod_path)
        self.make(stage = 'extract', foosched_tmpdir = self.tmp_dir, foosched_modpath = self.mod_path,
                  objs = self.mod_objs)
    ...
    def create_sandbox(self, kernel_src):
        ...
        for f, t in self.file_mapping.items():
            self.mod_sh.cp(glob(f, _cwd=self.foosched_path), t, recursive=True, dereference=True)
    ...
    def cmd_init(self, kernel_src, sym_vers, kernel_config):
        ...
        self.foosched_sh.cp(sym_vers,      self.work_dir, force=True)
        self.foosched_sh.cp(kernel_config, self.work_dir + '/.config', force=True)
        self.foosched_sh.cp(self.makefile, self.work_dir, force=True)
        self.foosched_sh.cp(self.vmlinux,  self.work_dir, force=True)
    ...
    def cmd_build(self):
        if not os.path.exists(self.work_dir):
            logging.fatal("foosched: Can't find %s", self.work_dir)
        self.add_python_path()
        logging.info("Preparing rpmbuild environment")
        rpmbuild_root = os.path.join(self.foosched_path, 'rpmbuild')
        self.foosched_sh.rm('rpmbuild', recursive=True, force=True)
        self.foosched_sh.mkdir('rpmbuild')
        rpmbase_sh = sh(_cwd=rpmbuild_root)
        rpmbase_sh.mkdir(['BUILD','RPMS','SOURCES','SPECS','SRPMS'])

        self.foosched_sh.cp('module-contrib/scheduler.spec', os.path.join(rpmbuild_root, 'SPECS'), force=True)
        rpmbase_sh.rpmbuild('--define', '%%_outdir %s' % os.path.realpath(self.foosched_path + '/module-contrib'),
                            '--define', '%%_topdir %s' % os.path.realpath(rpmbuild_root),
                            '--define', '%%_dependdir %s' % os.path.realpath(self.foosched_path),
...
if __name__ == '__main__':
    ...
    if arguments['extract_src']:
        ...
        foosched = Foosched(work_dir, vmlinux, makefile)
        foosched.cmd_init(kernel_src, sym_vers, kernel_config)
