...
#define foosched_cpuset_lock() \
	mutex_lock(&cpuset_mutex)
#define foosched_cpuset_unlock() \
...
struct kobject *foosched_dir, *foosched_subdir, *vmlinux_moddir;
...
static inline void install_sched_domain_sysctl(void)
{
	...
	foosched_cpuset_lock();

	__orig_unregister_sched_domain_sysctl();
	register_sched_domain_sysctl();

	foosched_cpuset_unlock();
...
static ssize_t foosched_enabled_store(struct kobject *kobj,
...
static ssize_t foosched_enable_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", scheduler_enable);
}

static struct kobj_attribute foosched_enable_attr =
	__ATTR(enable, 0644, foosched_enable_show, foosched_enabled_store);

static int register_foosched_enable(void)
{
	int ret = -ENOMEM;

	foosched_dir = kobject_create_and_add("foosched", kernel_kobj);
	if (!foosched_dir)
		return -ENOMEM;

	foosched_subdir = kobject_create_and_add("foosched", foosched_dir);
	if (!foosched_subdir)
		goto error;

	vmlinux_moddir = kobject_create_and_add("vmlinux", foosched_subdir);
	if (!vmlinux_moddir)
		goto error;

	ret = sysfs_create_file(foosched_subdir, &foosched_enable_attr.attr);
	if (ret)
		goto error;
...
error:
	...
	kobject_put(foosched_subdir);
	kobject_put(foosched_dir);
	return ret;
}

static void unregister_foosched_enable(void)
{
	sysfs_remove_file(foosched_subdir, &foosched_enable_attr.attr);
	kobject_put(vmlinux_moddir);
	kobject_put(foosched_subdir);
	kobject_put(foosched_dir);
}
...
static inline void unregister_foosched_sysfs(void)
{
	unregister_tainted_functions();
	unregister_foosched_enable();
}

static int register_foosched_sysfs(void)
{
	if (register_foosched_enable()) {
		printk("scheduler: Error: Register foosched sysfs failed!\n");
		return -ENOMEM;
	}

	if (register_tainted_functions()) {
		printk("scheduler: Error: Register taint functions failed!\n");
		unregister_foosched_sysfs();
		return -ENOMEM;
	}
